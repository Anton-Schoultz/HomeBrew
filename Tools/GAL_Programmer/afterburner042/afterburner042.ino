...
void printHelp(char full) {
  ...
  Serial.println(F("  u - clear map and start upload of fuses into map (via #... lines)"));
  Serial.println(F("  w - write uploaded fuses (from map to chip)"));
  Serial.println(F("  v - verify fuses"));
  Serial.println(F("  c - erase chip"));
  Serial.println(F("  ~ - erase all (resets PES)"));
...
// Commands:
// t <gal index>: gal type index to the GALTYPEE enum         - '#t2'
...
void parseUploadLine() {
  switch (line[1]) {
    ...
    //fusemap data - #f aaaa ffff..
    // aaaa = 4 decimal digit address
    // ff ... fuse flags as hex
    case 'f': {
      char i = 8;
      char j;
...
void loop() {
    ...
    switch (command) {
      ...
      case COMMAND_UTX : { // '#' upload rows
        parseUploadLine();
      } break;

      // read and print the PES
      case COMMAND_READ_PES : { // 'p'
        char type;
        readPes();
      ...
      case COMMAND_ENABLE_SECURITY: { // 's'
        if (doTypeCheck()) {
          secureGAL();
        }
      } break;
      ...
      case COMMAND_TEST_VOLTAGE : { // 't'
        testVoltage(20);
      } break;

      case COMMAND_SET_GAL_TYPE : { // 'g'
        char type = line[1] - '0';
      ...
      case COMMAND_ENABLE_CHECK_TYPE: { // 'f'
        setFlagBit(FLAG_BIT_TYPE_CHECK, 1);
      } break;
      case COMMAND_DISABLE_CHECK_TYPE: { // 'F'
        setFlagBit(FLAG_BIT_TYPE_CHECK, 0);
      } break;
      default: {
        if (command != COMMAND_NONE) { // null
